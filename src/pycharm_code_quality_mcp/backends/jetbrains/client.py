...
本模块只读:仅调用 get_project_status 与 get_file_problems 两个白名单工具,
...
REQUIRED_TOOLS: frozenset[str] = frozenset({"get_project_status", "get_file_problems"})
...
class JetBrainsClient:
    ...
    async def get_project_status(self) -> dict[str, Any]:
        ...
        try:
            result = await asyncio.wait_for(
                session.call_tool("get_project_status", arguments=None),
    ...
    async def get_file_problems(
        ...
        """
        session = self._require_session()
        arguments: dict[str, Any] = {"fileAbsolutePath": file_path}
        if errors_only:
...
__all__ = ["ALLOWED_TOOLS", "REQUIRED_TOOLS", "JetBrainsClient"]
